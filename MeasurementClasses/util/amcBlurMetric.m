function blur = amcBlurMetric(im,ksiz)
...
B_Ver = imfilter(im,Hv,'same','replicate');%blur the input image in vertical direction
B_Hor = imfilter(im,Hh,'same','replicate');%blur the input image in horizontal direction

D_F_Ver = abs(im(:,1:x-1) - im(:,2:x));%variation of the input image (vertical direction)
D_F_Hor = abs(im(1:y-1,:) - im(2:y,:));%variation of the input image (horizontal direction)
