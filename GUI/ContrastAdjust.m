classdef ContrastAdjust < handle
    ...
    methods
        function obj = ContrastAdjust(numCh,parent,initxx,inityy,chanidx)
            ...
            if nargin>0 && ~isempty(numCh)
                obj.numCh = numCh;
                obj.xx = repmat({[0;1]},[obj.numCh,1]);
                obj.yy = repmat({[0;1]},[obj.numCh,1]);
                
                for ii = 1:obj.numCh
                    obj.procArray{ii} = CurvesLUT(xlim,255,obj.xx{ii},obj.yy{ii});
                end
            else
                obj.numCh = 0;
            end
...
% %                 set([obj.bh{ii},obj.axArray(ii)],'ButtonDownFcn',{@obj.bdownfun,ii})
                ...
                temptitles{ii} = sprintf('Channel %d',obj.chanIdx(ii));
...
%             set(obj.tabh,'TabTitles',temptitles)
        ...
        function rangeeditcallback(obj,src,evt,ii)
            ...
            temphigh = str2double(get(obj.highedit(ii),'String'));
