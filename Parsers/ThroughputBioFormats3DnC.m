classdef ThroughputBioFormats3DnC < ThroughputParser
...
% %         OtherDimensions
    ...
    methods
        function this = ThroughputBioFormats3DnC(parserobj)
    ...
    %             temp = joinDimensions(temp,{1,[2,3]});
                ...
                for jj = 1:numel(cvals)
                    for kk = 1:numel(zvals)
                        imind = this.ParserObj.calcImageIndex(tidx,cvals(jj),zvals(kk));
                        
                        temp = bfGetPlane(this.ParserObj.BFReader,imind);
                        if kk==1
                            imdata{jj} = zeros([size(temp,1),size(temp,2),numel(zvals)],'like',temp);
                        end
                ...
                chanvals = cvals;
                imobj{ii} = cImage3DnCNoFile(imdata,this.ParserObj.NativeColours(chanvals),...
                    chanvals, [this.ParserObj.PixelSize, this.ParserObj.PixelSize,this.ParserObj.ZDistance],...
                    imageLabels{1});
