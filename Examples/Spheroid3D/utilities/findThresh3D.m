function thresh = findThresh3D(im3d,imbg)
    % replacement for the old stackshow function which used GUIDE and was
    % quite buggy..

    cmap = gray(255);
    
    if nargin<2 || isempty(imbg)
        imbg = rangeNormalise(im3d);
    end
    ...
    % need additional text boxes for these
    opacSlider = uicontrol('parent',vb2,'style','slider','min',0,'max',1,...
        'value',0.5,'callback',@opaccallback,'sliderstep',[0.1,1]);
    
    thresh = 0.5*(min(im3d(:))+max(im3d(:)));
    
    threshSlider = uicontrol('parent',hb,'style','slider','min',min(im3d(:)),...
        'max',max(im3d(:)),'value',thresh,...
        'callback',@threshcallback);
    ...
    set(vb2,'heights',[-1,50,40])
    
    set(controlbox,'widths',[-1,-1,-1,-1,80,100])
    set(hb,'widths',[50,-1,50])
    ...
    function updatedisplay()
        switch currdim
            case 1
                im2d = permute(squeeze(im3d(currslice(1),:,:)>=thresh),[2,1]);
                bg2d = permute(squeeze(imbg(currslice(1),:,:)),[2,1]);
                ijinds = [3,2];
            case 2
                im2d = squeeze(im3d(:,currslice(2),:)>=thresh);
                bg2d = squeeze(imbg(:,currslice(2),:));
                ijinds = [1,3];
            case 3
                im2d = im3d(:,:,currslice(3))>=thresh;
                bg2d = imbg(:,:,currslice(3));
                ijinds = [1,2];
        end
        
        frac = get(opacSlider,'Value');
        newcdata = frac*im2d + (1-frac)*bg2d;
...
%         imlim = [min(im3d(:)),max(im3d(:))];
        ...
        if isempty(imh)
            imh = imagesc('parent',ax,'cdata',newcdata);
        else
            set(imh,'cdata',newcdata)
        end
        set(ax,'clim',[0,1],'xlim',[1,imsiz(ijinds(2))],'ylim',[1,imsiz(ijinds(1))]);
    ...
    function threshcallback(src,evt)
        thresh = get(src,'Value');
        updatedisplay();
    end

    function opaccallback(src,evt)
        updatedisplay();
    end
