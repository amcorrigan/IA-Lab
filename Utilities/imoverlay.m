function out = imoverlay(in, mask, color, thickness)
%Yinhai: IMOVERLAY Create a mask-based image overlay.
%   OUT = IMOVERLAY(IN, MASK, COLOR, thickness) takes an input image, IN, and a binary
%   image, MASK, and produces an output image whose pixels in the MASK
%   locations have the specified COLOR, where thickness tells the width of
%   the mask.
%
...
% color value.
    mask = imdilate(mask, ones(thickness,thickness));
